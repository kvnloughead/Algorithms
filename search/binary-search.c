...
// Iterative binary search based on https://www.geeksforgeeks.org/binary-search/
int binarySearch(int* arr, int length, int target) {
  ...
  while (start <= end) {  // while length of subarray > 0
    // mid index is starting index + half of length of subarray
    int mid = start + (end - start) / 2;
    if (arr[mid] == target) {
      return mid;
    }
    if (arr[mid] < target) {
      // set current start index one to right of current mid index
      start = mid + 1;
    } else {
      // set current end index one to left of current mid index
      end = mid - 1;
    }
  }
  return -1;
...
int main(void) {
  ...
  // int x = 10;
  int x = 988;
  // int result = binarySearch(arr, 0, n - 1, end);
  // int result = binarySearch((int*)&arr, n, 997);
  // int result = binarySearch((int*)&arr, n, 997);
  int result = binarySearch((int*)&arr3, 6, 9);
  (result == -1) ? printf(
